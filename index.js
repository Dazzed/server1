...
const app = express();
const server = app.listen(port, console.log(`SERVER-1=>listening at PORT:${port}`));
...
async function setToRedis(key, data) {
  try {
    ...
    console.log('SERVER-1=>Error in (setToRedis):', err);
  }
}

app.get('/setRoom', async function (req, res) {
  try {
    console.log('SERVER-1=>Get request for (/setRoom)');
    const { query_room } = req.query;
    var rooms = [];
    rooms = JSON.parse(await getFromRedis('rooms'));
    console.log('SERVER-1=>Rooms available:', rooms);
    ...
    console.log('SERVER-1=>Error in (/setRoom):', err);
  }
});
...
app.get('/getChats', async function (req, res) {
  try {
    ...
    console.log('SERVER-1=>Error in(/getChats):', err);
  }
});
...
amqp.connect(ampqUrl, function (err, conn) {
  conn.createChannel(function (err, channel) {
    ...
    io.on('connection', socket => {
      console.log('SERVER-1=>Client connected with Id:', socket.id);
      socket.on('chat', data => {
        const chat = {
          roomId: data.roomId,
          handle: data.handle,
